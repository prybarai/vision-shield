...
export default function WorkingVisionStart() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const locationType = ['exterior', 'deck', 'roof', 'landscaping'].includes(category) ? 'exterior' : 'interior';
      ...
      const projectResponse = await fetch('/api/projects/create', {
        ...
        body: JSON.stringify({
          ...
          style_preference: 'modern', // Default style
          quality_tier: 'mid', // Default quality
          notes: description,
          // session_id will be generated by the API if not provided
        }),
      });
      ...
      if (!projectResponse.ok) {
        ...
        throw new Error('Failed to create project. Please check your inputs and try again.');
      }

      const projectData = await projectResponse.json();
      const projectId = projectData.project?.id || projectData.project_id;
      ...
      formData.append('file', blob, 'project-photo.jpg');
  ...
  const categories = [
    { id: 'landscaping', label: 'Landscaping / Yard', icon: <Trees className="h-5 w-5" /> },
    { id: 'exterior_paint', label: 'Exterior Paint', icon: <Home className="h-5 w-5" /> },
    { id: 'deck_patio', label: 'Deck / Patio', icon: <Trees className="h-5 w-5" /> },
    { id: 'roofing', label: 'Roof', icon: <Building className="h-5 w-5" /> },
    ...
    { id: 'flooring', label: 'Flooring', icon: <Home className="h-5 w-5" /> },
  ];
